...
/* Modified by Ebrahim Mohammadi for use among CodeMirror */

function jsToolBar(codemirror, visiblearea) {
	if (!document.createElement) { return; }
	
	if (!codemirror || !visiblearea) { return; }
	
	if ((typeof(document["selection"]) == "undefined")
	&& (typeof(codemirror["setSelection"]) == "undefined")) {
		return;
	}
	
	this.codemirror = codemirror;
	this.textarea = codemirror.getInputField();
	
	this.editor = document.createElement('div');
	this.editor.className = 'jstEditor';
	
	visiblearea.parentNode.insertBefore(this.editor,visiblearea);
	this.editor.appendChild(visiblearea);
...
jsToolBar.prototype = {
	...
	encloseLineSelection: function(prefix, suffix, fn) {
		...
		if (typeof(document["selection"]) != "undefined") {
			...
			range.select();
		} else if (typeof(this.codemirror["setSelection"]) != "undefined") {
			this.codemirror.replaceSelection(subst);
			if (sel) {
				this.codemirror.setSelection(start + subst.length, start + subst.length);
			} else {
				this.codemirror.setSelection(start + prefix.length, start + prefix.length);
			}
